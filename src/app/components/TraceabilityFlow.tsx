...
export function TraceabilityFlow({ nodes }: TraceabilityFlowProps) {
  return (
    <section className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/70 md:p-6">
      ...
      <div className="mt-5 flex gap-3 overflow-x-auto pb-2">
        {nodes.map((node, index) => (
          <div key={`${node.label}-${node.value}`} className="flex items-center gap-3">
