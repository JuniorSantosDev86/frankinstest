...
public class AnthropicAiProvider implements AiProviderPort {
    ...
    public AiProviderResponse generate(String prompt) {
        if (apiKey == null || apiKey.isBlank()) {
            return AiProviderResponse.failure(
                "ANTHROPIC_API_KEY não configurada. Configure a variável de ambiente para usar IA real.");
        }
        ...
        try {
            ...
            if (response.statusCode() != 200) {
                return AiProviderResponse.failure(
                    "Anthropic API retornou status " + response.statusCode());
            }

            return parseResponse(response.body());
        } catch (Exception e) {
            return AiProviderResponse.failure("Erro ao chamar Anthropic API: " + e.getMessage());
        }
    }
    ...
    private AiProviderResponse parseResponse(String responseBody) throws Exception {
        ...
        if (!content.isArray() || content.isEmpty()) {
            return AiProviderResponse.failure("Resposta da Anthropic API não contém conteúdo");
        }
